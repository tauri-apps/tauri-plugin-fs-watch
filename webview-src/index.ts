...
export async function watch(paths: string | string[], options: DebouncedWatchOptions, cb: (event: DebouncedEvent) => void) {
  const opts = {
    recursive: false,
    delayMs: 2000,
    ...options,
...
export async function watchImmediate(paths: string | string[], options: WatchOptions, cb: (event: RawEvent) => void) {
  const opts = {
    recursive: false,
    ...options,
    delayMs: null
